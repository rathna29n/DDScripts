...
await page.locator('#applicationType_chosen').getByText('Cancel a Restriction', { exact: true }).click();
await page.waitForTimeout(3000);
...
//Check the Fee Values and Fee should display as zero
  const fee_value_locator:string="xpath=(//input[@ng-readonly='ReadonlyCalculator(orderLine.productSupplierCode,orderresponses.questionId, orderresponses)'])[2]";
  const fee_value_label:string="fee_value";
  await checkAutoPopulatedFeeValues(page,fee_value_locator,fee_value_label);

 //checking the fee
    const fee_locator:string="xpath=(//input[@ng-readonly='ReadonlyCalculator(orderLine.productSupplierCode,orderresponses.questionId, orderresponses)'])[1]";
    const fee_label:string="Fee";
    await checkAutoPopulatedFeeValues(page,fee_locator,fee_label);
...
//Place Order
  ...
  const OrderId:string =  order_Details_Header.split(" ")[1];
...
async function checkAutoPopulatedFeeValues(page,locatorname:string,logelement:string) {
  const fee_elements = await page.$$(locatorname);

  if (fee_elements.length > 0) {
    const inputValue = await fee_elements[0].inputValue();
    // Print the value to the console
    console.log(logelement+'=', inputValue);
  } else {
    console.log(logelement+'=',"No matching element found");
  }
}
